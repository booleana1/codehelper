...
import { CheckCircle2, Loader2, Wand2, MessageSquareWarning, Lightbulb, ListChecks, ArrowRight, Sparkles, HelpCircle } from 'lucide-react';
...
const renderFormattedText = (text: string | undefined) => {
  ...
  return lines.map((line, lineIndex) => {
    ...
    if (line.startsWith('## ')) {
      return <h2 key={lineIndex} className="text-xl font-semibold mt-3 mb-1 text-primary">{line.substring(3)}</h2>;
    }
    // Handle List Items (e.g., - item or * item)
    if (line.startsWith('- ') || line.startsWith('* ')) {
      const itemText = line.substring(2);
      const parts = itemText.split(/(\*\*.*?\*\*|\`.*?\`)/g).filter(Boolean);
      return (
        <div key={lineIndex} className="mb-1 flex items-start">
          ...
          <p>
            {parts.map((part, index) => {
              ...
              if (part.startsWith('\`') && part.endsWith('\`')) {
                return <code key={index} className="bg-muted px-1 py-0.5 rounded font-mono text-sm text-foreground/80">{part.substring(1, part.length - 1)}</code>;
              }
              return part;
            })}
          </p>
    ...
    // Handle regular paragraph lines with inline formatting
    const parts = line.split(/(\*\*.*?\*\*|\`.*?\`)/g).filter(Boolean);
    return (
      <p key={lineIndex} className="whitespace-pre-wrap mb-1">
        {parts.map((part, index) => {
          if (part.startsWith('**') && part.endsWith('**')) {
            return <strong key={index}>{part.substring(2, part.length - 2)}</strong>;
          }
    ...
    <div className="grid md:grid-cols-2 gap-8 h-[80vh]">
      <Card className="shadow-lg flex flex-col min-h-0">
        <CardHeader>
          ...
          <CardDescription>
             Cole seu código, descreva o problema ou faça uma pergunta geral sobre desenvolvimento.
          </CardDescription>
        </CardHeader>
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-xl font-semibold mb-3 flex items-center gap-2 text-primary">
                    <Sparkles className="h-6 w-6" /> Conselho do Mentor
                  </h3>
                      ...
                      {currentAnalysis.guidance.split(/\\n|\n/).map((step, index) => {
                        ...
                        return (
                          <div key={index} className={`mb-1 flex items-start`}>
                            ...
                            <span>{renderFormattedText(trimmedStep)}</span>
                          </div>
                        );
                      })}
                    </div>
